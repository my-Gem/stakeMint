...
contract stakeMint is ReentrancyGuard {
    ...
    // 质押挖矿触发的事件
    event StakeMint(address indexed owner, uint256 amount, uint256 totalUsdt);
    ...
    constructor(address usdt, address gbc) {
        USDT = usdt;
        GBC = gbc;
    }
    ...
    function getPower(address user) public view returns (uint256) {
        return
            BASICCOMPUTINGPOWER + (minter[user].totalUsdt / 100e6) * HASHRATE;
    }
